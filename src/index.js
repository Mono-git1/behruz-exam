    ...
    try {
        ...
        if (!adminCheck.rows[0].is_admin) {
            return res.status(403).send({
                massage: "Faqat admin foydalanuvchilar kirish mumkin! "
            })
        }
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (datauser.rows.length <= 0) {
            return res.status(400).send({
                massage: "Bunday user mavjud emas"
            })
        }
        try {
            if (await argon2.verify(datauser.rows[0].password, password)) {
                ...
                return res.status(400).send({ massage: "parol yoki emailda hatolik! " })
            }
        } catch (err) {
            return res.status(400).send({ massage: "parol tekshirish xatoligi! " })
        }
    } catch (error) {
